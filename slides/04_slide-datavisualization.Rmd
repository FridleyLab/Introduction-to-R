...
Aesthetic is a visual property of the objects in a plot and include attributes like size, shape, or color of the objects in your plot. 
...
These two codes generate the same bar plot

Typically we can use geoms without specifying stats but we might need to explicitly specify stats to override the default stat or to override the default mapping from transformed variables to aesthetics

---
### [stat catalogue](https://ggplot2.tidyverse.org/reference/index.html#section-layers)
...
ggplot(
  ...
  geom_point() +
  scale_x_continuous(
    "Weights (lbs)", limits = c(40, 180), breaks = seq(40, 180, 20)) + 
  scale_y_continuous(
    "Heights (cm)", limits = c(120, 200), breaks = seq(120, 200, 20)) + 
  scale_color_discrete(
    "Sex", na.value = "red")
```
]
...
drop_na(pancancer, weight, height) %>% 
  ggplot(
    mapping = aes(x = weight, y = height)) +  
  geom_point() +
...
Coordinate system, `coord` for short, maps the position of objects onto the plane of the plot
...
- Non-linear coordinate systems that can change the shape of geoms
...
```{r zoomin, echo = TRUE, fig.show = "hide"}
drop_na(pancancer, weight, height) %>% 
  ggplot(
    mapping = aes(x = weight, y = height)) +  
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  ggpubr::stat_regline_equation(label.x = 120, label.y = 135)
```
]

.panel[.panel-name[Plot] 

```{r ref.label = "zoomin", echo = FALSE, fig.height = 3}
```
]
]

Say, we want to zoom into plot with weight between 60 and 100 kgs


---
Setting `limits` arg in `scale_x_continuous()`

.panelset[
.panel[.panel-name[R Code]

```{r zoomin2, echo = TRUE, warning = TRUE, fig.show = "hide"}
drop_na(pancancer, weight, height) %>% 
  ggplot(
    mapping = aes(x = weight, y = height)) +  
  geom_point() +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  ggpubr::stat_regline_equation(label.x = 80, label.y = 135)
```
...
```{r ref.label = "zoomin2", echo = FALSE}
```
]
]

.blue[*any data outside of the limits is discarded*]
---
Setting `xlim` arg in `coord_cartesian()`

.panelset[
.panel[.panel-name[R Code]

```{r zoomin3, echo = TRUE, warning = TRUE, fig.show = "hide"}
drop_na(pancancer, weight, height) %>% 
  ggplot(
    mapping = aes(x = weight, y = height)) +  
  geom_point() +
...
```{r ref.label = "zoomin3", echo = FALSE}
```
]
]

.blue[*we use all the data but only the region of the plot we want is displayed*]

---
#### Flipping the axes with `coord_flip()`

Exchanging axes rotates the plot 90 degrees, but the smooth is fit to the rotated data


.panelset[
.panel[.panel-name[R Code]

```{r coord_flip, echo = TRUE, fig.show = "hide"}
drop_na(pancancer, weight, height) %>% 
  ggplot(
    mapping = aes(y = weight, x = height)) +  #<<
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  ggpubr::stat_regline_equation(label.x = 130, label.y = 160)
```
]

.panel[.panel-name[Plot] 

```{r ref.label = "coord_flip", echo = FALSE}
```
]
]
...
```{r coord_flip2, echo = TRUE, fig.show = "hide"}
drop_na(pancancer, weight, height) %>% 
  ggplot(
    mapping = aes(x = weight, y = height)) +  
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  ggpubr::stat_regline_equation(label.x = 160, label.y = 130) +
  coord_flip() #<<
```
]
...
```{r ref.label = "coord_flip2", echo = FALSE}
```
]
]
...
drop_na(pancancer, weight, height) %>% 
  ggplot(mapping = aes(x = weight, y = height, color = gender)) +  
  geom_point() +
  facet_wrap(~ tumor_tissue_site, nrow = 1) +
...
`r chunk_reveal("ggplotflipbook", widths = c(50, 50))`